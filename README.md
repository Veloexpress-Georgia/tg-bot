...
Before each background cycle, the bot checks the tracked poll messages. If all
polls for a future service day are confirmed missing, it silently cancels that
day, deletes its availability card, and preserves payment entries plus the refund
estimate through the normal cancellation reconciliation. This also applies on
the service day before its first scheduled departure. Once departures have
started, deleting an old poll is treated as possible chat cleanup and does not
automatically cancel the day or imply refunds. A missing status message alone
never cancels a day. Telegram access/network failures are not deletion evidence.
...
`☰ Menu` holds everything that is not the day in hand: `📋 Weekend`, `➕ Extra lift day`, `📜 History`, `📈 Statistics`, `⚙️ Settings` and `🧾 Refunds`, with a button back to the current day. `⚙️ Settings` changes the price in 1 GEL steps and the booking deadline in 30-minute steps without a redeploy, and says in full that both apply to days published from then on while days already published keep the terms they opened with. `/create_lift_poll` still jumps straight to the weekend card but is no longer suggested. Polls are always posted to `TELEGRAM_TARGET_CHAT_ID` and, when configured, `TELEGRAM_TARGET_THREAD_ID`.

`📋 Weekend` is the single source of truth for what gets posted: it shows the upcoming weekend, the lift-time range (learned from recent weeks), which days are enabled, and when the polls will open automatically. Edits are saved as a plan — nothing posts until the scheduled time or an explicit `🚀 Post now`. Posted days are marked and protected; `♻️ Recreate polls` (with confirmation) replaces an already-posted weekend and reports tracked votes first. A `⏭ Skip weekend` toggle suppresses one auto run, and lives only here: offering it from the schedule card as well was two buttons for one flag, naming the weekend differently in each place.
...
The day card opens with two short lines rather than a row of figures: how many lifts are running out of those still active and how many of them are funded, then the money as reported-of-expected. Reaching the rider minimum and being paid for are never printed as the same thing — five riders and no payments is running, not funded. Seats never exceed capacity: a lift with twelve bookings in a ten-seat van reads `10/10 seats · 2 waiting`, because `12/10` was read as twelve people on board. Riders who reported nothing, riders who reported cash, the waitlist, late exits and guests each get one line and are never totalled together — the old `Needs attention · N` counted one rider twice when they were in two categories, and `Cash to collect` called money a debt that the rider had already handed over and the ledger had already received. A day that has already run stays visible for late bookkeeping but loses its `🚫 Cancel day` button: cancelling reports every payment as a refund, which for a trip people actually took would hand back money that was earned. When a new weekend is posted, every admin's monitor is re-posted rather than edited, so it arrives at the bottom of the chat instead of staying buried where it was last opened; only admins who have opened it at least once can be reached, because Telegram refuses a private message to anyone who never started a chat with the bot.

Every lift of the day is a button on the day card, short ones included, and opens a single screen that both reads and acts: who is on it, what it is made of, how the money stands, `➖`/`➕ Offline seat` for people booked outside Telegram, and `🚫 Cancel lift` / `♻️ Restore lift` — a single lift is reversible. There is no intermediate "manage bookings" card; it listed the same times a second time and cost a tap on the way to every one of them. `👥 All riders` stays, because reading the whole day's roster at once is its own job. Before a cancellation the confirmation shows who is affected and a read-only refund estimate; backing out files nothing, since an admin who asks what a cancellation would cost has not cancelled anything. `🚫 Cancel day` retires the whole day: the polls close, the availability card is deleted, the day leaves the monitor, and the date is freed so an admin can post a fresh poll if it comes back (there is no in-place "restore day"). The day takes its hand-added riders and its payments with it — nobody can tell whether a manually added rider still intends to come, and the money is going back, so a revived poll opens genuinely empty rather than with seats and cash the bot no longer holds. Cancelling a lift marks it `❌ cancelled` on the public board; cancelling the day removes its availability card, leaving the separate cancellation notice. Either way the bot immediately tags the affected voters in the group thread so they know it is off. Public availability counts Telegram votes, manual bookings and guest seats, and shows each lift's state in words (`needs N more`, `N left`, `full`, `waitlist +N`, `❌ cancelled`). A lift with a waitlist names it on the next line, so `waitlist +2` stops being a riddle; the board is edited in place and a Telegram edit sends no notification, so naming informs without pinging. The seat order itself lives in one pure module used by both the board and the payments side, because two copies of that rule would drift and then disagree about who owes money. An admin must open the private bot chat before Telegram will allow the monitor to be delivered.
