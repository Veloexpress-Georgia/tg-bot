...
Afterwards the roster is laid over the live day and seats may only grow. Growing keeps a late booking chargeable, because nothing stops one and Misho may well allow it. Not shrinking is the point: a rider who drops out at half past eight no longer turns into `15 back`, and a lift that reached five no longer reads `needs 1 more` when two people leave. The single exception is a lift the admin cancels after the deadline — that is the one refund the group does recognise, so those rows are dropped from the overlay. A day whose poll was created after its own deadline is never frozen, since it never had one.
