...
One `PaymentTerms` object carries the money rules — price, deadline, link — so the poll notice, the "lift is running" message and the payments board all state them identically instead of drifting apart. The poll notice says a lift runs from five riders and that there is nothing to pay before the ✅ message; the ✅ message repeats the price and deadline, because crossing the minimum is exactly when payment falls due. The link points at the payments topic itself, derived from the chat and thread ids, so there is nothing to configure and nothing to keep in sync; with no payments topic set the link line is simply omitted.
...
`BOOKING_DEADLINE_TIME` is when booking and paying closes, on the evening *before* a lift day. Two hours before it, each service day gets one reminder in the lift topic naming the deadline and what every lift still needs. It is sent once per day and only while at least one lift is short of the minimum — when everything already runs there is nothing to ask the group for, and when everything is cancelled there is nothing to save. The reminder tags nobody: whoever is booked is already booked, and tagging the whole group is the spam this bot exists to avoid.
...
Guests open a form in the bot's private chat rather than a button on the board: only a form can say how many seats a lift has left, and only a private chat has room for one row per lift. The board links there, and the link is a deep link — tapping it *is* pressing Start, which is how a rider who never opened the bot ends up with a private chat the bot may write to. Bringing someone for the whole day is one tap; the per-lift rows are for a guest who only rides some laps, and vanish when the host holds a single lift. A full lift offers no plus button. Guest seats are per lift because a seat is capacity, and capacity belongs to a lift.
