...
One `PaymentTerms` object carries the money rules — price, deadline, `PAYMENT_LINK` — so the poll notice, the "lift is running" message and the payments board all state them identically instead of drifting apart. The poll notice says a lift runs from five riders and that there is nothing to pay before the ✅ message; the ✅ message repeats the price and deadline, because crossing the minimum is exactly when payment falls due. `PAYMENT_LINK` is optional and every message simply omits the link line when it is empty.
